...
<div>
   ...
   <form class="form-control" v-if="postForm" @submit.prevent="post" @keydown="form.onKeydown($event)">
 ...
 <div class="tweetEntry-tweetHolder mt-3">
   <post v-for="(post,index) in posts" :post="post" :index="index" :updateCommentform="updateCommentform" v-bind:key="post.id" @delete-post="deletePost(...arguments)" @init-post="initUpdatePost(...arguments)" @init-comment="initUpdateComment(...arguments)" :ref="'post' + index"></post>
 ...
 <!--End of tweetHolder-->
   ...
   <!-- <b-button @click="showModal">
    ...
    <b-modal ref="myCommentsModalRef" hide-footer title="Edit Comment">
      <form class="form-control" @submit.prevent="updateComment" @keydown="updateCommentform.onKeydown($event)">
        <div class="form-group">
         <textarea class="form-control" v-model="updateCommentform.body" :class="{ 'is-invalid': updateCommentform.errors.has('body') }" rows="1"></textarea>
         <has-error :form="updateCommentform" field="body"/>
       </div>
         <div class="form-row align-items-center">
         <div class="col-auto my-1">
           <v-button :loading="updateCommentform.busy">
                    Edit
           </v-button>
           <button type="button" class="btn btn-default" data-dismiss="modal" block @click="$refs.myCommentsModalRef.hide()">Close</button>
         </div>
       </div>
       </form>
    </b-modal>
...
methods: {
  ...
  deletePost: function(post,index) {
    ...
    async updateComment()
    {   
      this.$children[this.updateCommentform.post_index].updateComment();
